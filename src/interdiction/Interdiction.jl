...
function interdiction_flow{T<:AbstractFloat}(
  ...
  solver::AbstractMathProgSolver =               # keyword for solver
    UnsetSolver(),
  rtol::T = sqrt(eps()),                         # absolute tolerance
  atol::T = 0.,                                  # relative tolerance
  time_limit::Float64 = Inf                      # time limit (seconds)
  )
  # attacks ≥ λ (connectivity) → f = 0
  λ = maximum_flow(flow_graph, source, target)[1]
  (attacks ≥ λ) && return empty_flow(capacity_matrix, PushRelabelAlgorithm())
  ...
  return [
    interdiction_flow(flow_graph, source, target, capacity_matrix, attacks, algorithm,
